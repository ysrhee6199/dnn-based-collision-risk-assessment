...
sfmxLayer->getOutputTensor(0); CUDNN_CALL(cudnnSetTensor4dDescriptor( 
rytJDHzuydvYOLNNROYf, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, 
ipTensor->getSequenceLength() * ipTensor->getBatchSize(), 
ipTensor->getChannels(), ipTensor->getHeight(), ipTensor->getWidth())); 
CUDNN_CALL(cudnnSetTensor4dDescriptor( sFIUeCwGDlfadqOrGZHC, 
CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, opTensor->getSequenceLength() * 
opTensor->getBatchSize(), opTensor->getChannels(), opTensor->getHeight(), 
opTensor->getWidth())); } void MWSoftmaxLayerImpl::predict() { MWCNNLayer* 
sfmxLayer = getLayer(); MWTensorBase* ipTensorBase = 
...
CUDNN_CALL(cudnnSoftmaxForward(*cRtIUoZRPICuQEOZOSzT->getCudnnHandle(), 
